...
const fixture = path.join(import.meta.dir, "fixtures/sjawhar/commands/no-excuses.md");

describe("translateCommandFile", () => {
  test("wraps body in command-instruction + user-request envelope", async () => {
    const result = await translateCommandFile(fixture, { namespace: "sjawhar" }, logger);
    expect(result?.name).toBe("sjawhar-no-excuses");
    expect(result?.config.description).toBe("Reject excuse-making in the current turn and refocus on the goal.");
