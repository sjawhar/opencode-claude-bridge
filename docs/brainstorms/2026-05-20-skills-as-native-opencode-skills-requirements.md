...
- **R7.** The bridge materializes a normalized copy of every bridge-imported SKILL.md into a bridge-owned cache directory. The materialized copy has:
  ...
  - All other frontmatter passed through unchanged. Bridge does not strip Claude-Code-specific fields opencode ignores; opencode treats unknown frontmatter as ignored per spec.
